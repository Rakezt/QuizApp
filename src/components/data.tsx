export const quizData = [
  {
    question: "Who won the Premier League 2023/2024?",
    options: ["Arsenal", "Manchester United", "Liverpool", "Manchester City"],
    correctOption: 4,
  },
  {
    question: "Who is the winner of Fifa World Cup 2022?",
    options: ["Brazil", "Argentina", "Spain", "England"],
    correctOption: 1,
  },
  {
    question: "Who is the golden boot winner of EPL 2023/2024?",
    options: ["Mo salah", "Son", "Haaland", "Hoej'lund"],
    correctOption: 2,
  },
  {
    question: "Where will the upcoming Euro 2024 take place",
    options: ["England", "Span", "Russia", "Germany"],
    correctOption: 3,
  },
  {
    question: "Which team is the current winner of Champions League",
    options: ["Real Madrid", "Juventus", "Manchester City", "Barcelona"],
    correctOption: 0,
  },
];
